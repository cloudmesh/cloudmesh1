...
class keystone(object):
    ...
    def add_role_to_user_tenant(self, tenantid, userid, roleid):
        conf = self._get_service_endpoint("identity")
        adminURL = conf['adminURL']
        url = "%s/tenants/%s/users/%s/roles/OS-KSADM/%s" % (adminURL, tenantid, userid, roleid)
    ...
    def change_own_password(self,credential,userid,oldpass,newpass):
        ...
        if "access" in ret:
            ...
            ret = {"error":"failed to obtain auth_token using the provided credential"}
        return ret
    
    def test_change_own_password(self,cloudlabel,oldpass,newpass):
        mycredential = cm_config().get("cloudmesh.clouds.{0}.credentials".format(cloudlabel))
