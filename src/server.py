...
class server(object):
    ...
    ''' This approach is really bad, to delete a record among
     n uid's you need to rewrite n-1 uid's to file again (worst case) '''
    ...
    def list_server(self, uid):
        suid = 'server' + '.' + 'uid'
        cursor = self.collection.find({suid : uid})
        self.collection.find({'server.uid':'YgbYD6:127.0.0.1'})
