...
def launch_servers():
    ...
    usr = config["cloudmesh"]["clouds"][cloudname]["credentials"]\
    ["OS_USERNAME"]
    hostname = config["cloudmesh"]["clouds"][cloudname]["cm_host"] 
    ssh_cmd = "ssh " + usr + "@" + hostname + " \"" \
    + request.form['script'] + "\" >> /home/cloudnaut/results.txt"
...
def display_launch_table():
    ...
    launcher_config = ConfigDict(filename= \
    config_file("/cloudmesh_launcher.yaml"))
    launch_recipies = launcher_config \
    .get("cloudmesh.launcher.recipies") 
