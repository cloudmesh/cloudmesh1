...
# broken not sudo: azure("account", "import",
# "/Users/new/azure/.publishsettings")
...
# os.system("openssl x509 -inform pem -in mycert.pem -outform der -out
# mycert.cer")
def create_cert(name, organization, city, state, country, certname):
    ...
    data['city'] = country
    data[
        'cn'] = "/CN=%(name)s/O=%(organization)s/C=%(country)s/ST=%(state)s/L=%(city)s" % data
    # os.system("openssl req -x509 -subj \"/CN=Gregor von
    # Laszewski/O=IU/C=US/ST=Indiana/L=Bloomington\"  -nodes -days 365 -newkey
...
create_cert("Gregor von Laszewski", "IU",
            "Bloomington", "Indiana", "US", "mycert")
