...
class HTMLTranslator(SphinxHTMLTranslator):
    ...
    def visit_admonition(self, node, name=''):
        ...
        if name and name != 'seealso':
            node.insert(
                0, nodes.title(name, admonitionlabels[name], classes=['alert-heading']))
    ...
    def visit_bullet_list(self, node):
        ...
        if self.compact_simple and not old_compact_simple:
            ...
            if not self.is_partial and self.toc_subnav and node is self.page_toc:
                self.body.append(
                    self.starttag({'classes': ['page-toc', 'subnav', 'navbar']}, 'div'))
                self.body.append(
                    self.starttag({'classes': ['navbar-inner']}, 'div'))
                self.in_subnav = True
            ...
            # handle toc bullet list
                ...
                # only global toc is in the nav, so see if that is what we are
                # rendering
